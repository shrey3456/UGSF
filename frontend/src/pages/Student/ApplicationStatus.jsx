...
export default function ApplicationStatus() {
  ...
  // If submitted, show 'pending' only when HOD has marked it pending
  const label = status === 'submitted'
    ? (d?.hodMarkedPending ? 'pending' : 'submitted')
    : status
  const pill = {
    none: 'bg-slate-100 text-slate-700',
    submitted: d?.hodMarkedPending ? 'bg-yellow-100 text-yellow-800' : 'bg-slate-100 text-slate-700',
